...
if ($_SESSION['privileges'] == 1) {
            ?>
                <div class="btn-group">
                      ...
                      <a class="btn btn-primary active" href="commented.php">Kommentit <span class="fui-chat"></span></a>
...
<br><br><br><br>
		...
		Kuvat johon olet kommentoinnut.<br>
		<br>
<?php
        //haetaan kuvat ja sortataan se kommenttiajan perusteella (uusimmat ensin)
        $receive_pic = mysql_query("SELECT * FROM kommentit LEFT JOIN kuvat ON kommentit.kommentti_kuvan_id = kuvat.kuva_id LEFT JOIN users ON kommentit.kommentin_lisannyt = users.username WHERE users.id = '$user_id' ORDER BY kommentit.kommentti_lisaysaika DESC");
            //tarkistetaan että onko kuvia olemassa
            if (mysql_num_rows($receive_pic) == 0) {
                //kuvia ei löydetty
                echo 'Et ole vielä näköjään kommentoinnu mihinkään. kommentoi johonkin niin löydät kommenttisi myöhemmin täältä.';
            } else {
                //Hakee kuvat tietokannasta ($receive_pic) 
                while($row_pic = mysql_fetch_array($receive_pic)) {
                   echo '<div class="col-md-3"><a style="padding:10px;" href="kuva.php?kuva_id='.$row_pic["kuva_id"].'"><img style="border: 1px solid black;" width="148" height="128" alt="'.$row_pic["kuvan_teksti"].'" src="'.$row_pic["kuvan_linkki"].'"></img></a><p>&nbsp;&nbsp;&nbsp;&nbsp;Kommentti: &nbsp;"'.$row_pic["kommentin_sisalto"].'"</p></div>';
                }
            }
?>	
